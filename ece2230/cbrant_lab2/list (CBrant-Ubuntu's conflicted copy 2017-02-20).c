...
static void list_empty_insert(list_t lpoint, data_t dpoint, void *insert)
{
	struct list_node *add_node = (struct list_node *)insert;
...
data_t list_find(list_t lpoint, data_t dpoint, cmpfunc cmp)
{
	...
	for (i = 0; i < lpoint->list_size && found != 0; i++)
	{
		found = (*cmp)(lpoint->current, dpoint);
...
{
	data_t next_node = lpoint->current->tail;
	lpoint->current = next_node;
...
{
	data_t prev_node = lpoint->current->head;
	lpoint->current = prev_node;
...
{
	data_t last_node = lpoint->bottom;
...
int list_insert_before(list_t lpoint, data_t dpoint)
{
	...
	else
	{
		...
		lpoint->current->head->tail = new_node;
...
int list_insert_after(list_t lpoint, data_t dpoint)
{
	...
	else
	{
		...
		lpoint->current->tail->head = new_node;
